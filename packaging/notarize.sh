...
#   2. xcrun notarytool store-credentials baton-notary \
#        --apple-id "you@example.com" \
#        --team-id DAB6FR7R2R \
#        --password "xxxx-xxxx-xxxx-xxxx"
#
# Then:  ./packaging/notarize.sh dist/baton-local-<sha>-arm64.tar.gz
#
...
#   --options runtime --entitlements packaging/baton.entitlements
...
PROFILE="baton-notary"
TARBALL="${1:-}"

say()  { printf '\033[1m%s\033[0m\n' "$*"; }
fail() { printf '\033[31m%s\033[0m\n' "$*" >&2; exit 1; }

[ -n "$TARBALL" ] || fail "usage: $0 dist/baton-local-<sha>-arm64.tar.gz"
...
echo
say "verify with:"
echo "  tar -xzf $TARBALL -C /tmp && spctl -a -vvv -t install /tmp/\$(basename ${TARBALL%.tar.gz})/baton"
